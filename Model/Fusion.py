...
def calculate_metrics(y_true, y_pred_proba):
    ...
    return {
        ...
        'F1 Score': f1, 'MCC': mcc, 'AUC': auc_score, 'AUPR': aupr,
        'Precision': precision, 'Recall': recall, 'Accuracy': accuracy
    }

input_file_path = "./Model/Data/Rice_yield.csv"
...
raw = pd.read_csv(input_file_path, header=None).values
X_raw = raw
...
for fold_idx, (train_idx, valid_idx) in enumerate(kf.split(X)):
    ...
    fold_metrics = calculate_metrics(y_valid_fold, y_valid_proba)
    fold_metrics_list.append(fold_metrics)
    print(f"  Fold {fold_idx+1} completed - Val Loss: {history.history['val_loss'][-1]:.4f}")

    tf.keras.backend.clear_session()
    gc.collect()

avg_metrics = {metric: np.mean([f[metric] for f in fold_metrics_list]) for metric in fold_metrics_list[0]}
...
model_name = f"{os.path.splitext(file_name)[0]}_Fusion-model.h5"
save_path = os.path.join(output_folder_path, model_name)
final_model.save(save_path)
print(f"  Final model saved to: {save_path}")

del final_model, fold_metrics_list, avg_metrics
gc.collect()
tf.keras.backend.clear_session()

print("\nFusion process completed. Final model trained on full data has been saved.")
